...
**Season shows as monitored in Sonarr, but nothing shows up in Activity → Queue or History**
- Also should be fixed already — the season-level "monitored" flag is a rollup/display value; Sonarr's search only looks at each *episode's own* monitored flag underneath it, which a season-level update doesn't reliably cascade down to. The bot now sets episode-level monitoring explicitly and triggers a targeted per-season search (`SeasonSearch`) rather than a whole-series search. If a request still shows monitored with nothing in Queue/History afterwards, check the bot's logs for `failed to set season monitoring`, or just monitor the season directly from Sonarr's **Season Pass** page and use its "Search Season" button.

**Requests coming back "Sonarr never populated its episode list in time"**
- For a brand-new series, Sonarr populates its episode list via a background job right after creation rather than instantly — the bot waits up to ~12 seconds for that to finish before setting episode monitoring, and this message means it still hadn't after that (usually a slow/overloaded Sonarr instance, or it's still fetching metadata from TheTVDB). The series was still added to Sonarr, so `/request_show` won't let you retry it (it'll say "Already in Sonarr"). Instead, once Sonarr's finished its refresh, go into its **Season Pass** page and monitor + search the season(s) from there directly.
